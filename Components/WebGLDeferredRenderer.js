...
THREE.WebGLDeferredRenderer = function ( parameters ) {
	...
	const _this = thiconst
	const pixelWidth = parameters.width !== undefined ? parameters.width : 800;
	const pixelHeight = parameters.height !== undefined ? parameters.height : 600;
	const currentScale = parameters.scale !== undefined ? parameters.scale : 1;
	...
	const fullWidth = pixelWidth * devicePixelRatio;
	const fullHeight = pixelHeight * devicePixelRatio;
	
	const scaledWidth = Math.floor( currentScale * fullWidth );
	const scaledHeight = Math.floor( currentScale * fullHeight );
	
	const brightness = parameters.brightness !== undefined ? parameters.brightness : 1;
	const tonemapping = parameters.tonemapping !== undefined ? parameters.tonemapping : THREE.SimpleOperator;
	const antialias = parameters.antialias !== undefined ? parameters.antialias : false;
	...
	const currentCamera = null;
	const projectionMatrixInverse = new THREE.Matrix4();

	const positionVS = new THREE.Vector3();
	const directionVS = new THREE.Vector3();
	const tempVS = new THREE.Vector3();

	const rightVS = new THREE.Vector3();
	const normalVS = new THREE.Vector3();
	const upVS = new THREE.Vector3();

	//

	const geometryLightSphere = new THREE.SphereGeometry( 1, 16, 8 );
	const geometryLightPlane = new THREE.PlaneBufferGeometry( 2, 2 );

	const black = new THREE.Color( 0x000000 );

	 colorShader = THREE.ShaderDeferred[ "color" ];
	 normalDepthShader = THREE.ShaderDeferred[ "normalDepth" ];

	//

	const emissiveLightShader = THREE.ShaderDeferred[ "emissiveLight" ];
	const pointLightShader = THREE.ShaderDeferred[ "pointLight" ];
	const spotLightShader = THREE.ShaderDeferred[ "spotLight" ];
	const directionalLightShader = THREE.ShaderDeferred[ "directionalLight" ];
	const hemisphereLightShader = THREE.ShaderDeferred[ "hemisphereLight" ];
	const areaLightShader = THREE.ShaderDeferred[ "areaLight" ];

	const compositeShader = THREE.ShaderDeferred[ "composite" ];
	...
	const invisibleMaterial = new THREE.ShaderMaterial();
	invisibleMaterial.visible = false;


	const defaultNormalDepthMaterial = new THREE.ShaderMaterial( {
	...
	const initDeferredMaterials = function ( object ) {

		if ( object.material instanceof THREE.MeshFaceMaterial ) {

			const colorMaterials = [];
			const defaultnormalDepthMaterials = [];

			const materials = object.material.materials;
